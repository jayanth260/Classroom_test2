...
int main(int argc, char *argv[]){
	...
	try{
		...
		constexpr const float error_threshold = 10;
		std::uint32_t remaining = 0;
			...
			for (std::uint32_t i = 0; i+8 <= file_size; i += 8) {
				// printf("%f %f",student_data[i],sol_data[i]);
		...
		for(std::uint32_t i=file_size-remaining; i < file_size; i++){
			...
			if(diff > error_threshold){
				// printf("%f %f\n",sol_data[i],student_data[i]);
				__terminate_gracefully("Solution has higher error than required: " + std::to_string(diff));
				}
		}
